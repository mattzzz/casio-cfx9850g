...
def _local_dict(variables: dict, angle_mode: str = "DEG") -> dict:
...
def _eval(expr_str: str, variables: dict, angle_mode: str = "DEG") -> float:
    """Evaluate a Casio BASIC expression. Returns float. Raises ValueError on error."""
    s = expr_str.strip()
    if not s:
        raise ValueError("Empty expression")
    ld = _local_dict(variables, angle_mode)
    try:
...
def _eval_cond(cond: str, variables: dict, angle_mode: str = "DEG") -> bool:
    """Evaluate a boolean condition. Returns True/False."""
    s = cond.strip()
    # Normalise Unicode relational operators
    ...
    for op in (">=", "<=", "!=", ">", "<"):
        idx = s.find(op)
        if idx > 0:
            lv = _eval(s[:idx].strip(), variables, angle_mode)
            rv = _eval(s[idx + len(op):].strip(), variables, angle_mode)
            if op == ">=": return lv >= rv
            if op == "<=": return lv <= rv
            if op == "!=": return lv != rv
            if op == ">":  return lv >  rv
    ...
    if idx > 0:
        lv = _eval(s[:idx].strip(), variables, angle_mode)
        rv = _eval(s[idx + 1:].strip(), variables, angle_mode)
        return abs(lv - rv) < 1e-9

    # No comparison operator — numeric truth (non-zero)
    try:
        return _eval(s, variables, angle_mode) != 0.0
    except ValueError:
        return False
...
_ASSIGN_RE   = re.compile(r"^(.+?)\s*→\s*([A-Z])\s*$", re.DOTALL)
_FOR_RE      = re.compile(
    r"^For\s+([A-Z])\s*=\s*(.+?)\s+To\s+(.+?)(?:\s+Step\s+(.+))?$", re.IGNORECASE
)
_NEXT_RE     = re.compile(r"^Next(?:\s+([A-Z]))?$", re.IGNORECASE)
_WHILE_RE    = re.compile(r"^While\s+(.+)$", re.IGNORECASE)
_LPWHILE_RE  = re.compile(r"^LpWhile\s+(.+)$", re.IGNORECASE)
_LBL_RE      = re.compile(r"^Lbl\s+([0-9A-Za-z])\s*$", re.IGNORECASE)
_GOTO_RE     = re.compile(r"^Goto\s+([0-9A-Za-z])\s*$", re.IGNORECASE)
_INPUT_RE    = re.compile(r'^Input\s*(?:"([^"]*)"\s*,\s*)?([A-Z])\s*$', re.IGNORECASE)
_INPUT_Q_RE  = re.compile(r"^\?\s*→?\s*([A-Z])\s*$")
...
def parse_stmt(s: str, line_no: int = 0) -> Statement:
    ...
    if m:
        return Statement("INPUT", {"prompt": "?", "var": m.group(1).upper()}, raw, line_no)
    ...
    if m:
        return Statement("ASSIGN", {"expr": m.group(1).strip(), "var": m.group(2).upper()}, raw, line_no)
...
def parse_program(source: str) -> list[Statement]:
    ...
    for line_no, line in enumerate(source.splitlines(), 1):
        line = re.sub(r"\s*//.*$", "", line).strip()   # strip // comments
...
class CasioBasicInterpreter:
    ...
    def run(self, source: str, inputs: list | None = None) -> RunResult:
        """Execute *source* to completion.

        *inputs* is a list of pre-supplied values consumed by Input statements
        (useful for testing and batch execution).
        """
        stmts = parse_program(source)
        jt    = build_jump_table(stmts)
        res   = RunResult()
        inp   = list(inputs or [])
        self._execute(stmts, jt, res, inp, call_depth=0)
    ...
    def _execute(
        ...
        while ip < len(stmts):
            ...
            if s.cmd == "ASSIGN":
                try:
                    v = _eval(s.args["expr"], self.variables, self.angle_mode)
            ...
            # ── Print expression ─────────────────────────────────────────
                ...
                if expr.startswith('"') and expr.endswith('"'):
                    ...
                    try:
                        v = _eval(expr, self.variables, self.angle_mode)
                        self.variables["ANS"] = v
                        res.text(_fmt(v))
                    except ValueError as e:
                        res.text(str(e))
            ...
            # ── Input ────────────────────────────────────────────────────
                ...
                if inp_pos < len(inp):
                    ...
                    try:
                        ...
                        try:
                            v = _eval(str(raw), self.variables, self.angle_mode)
                        except ValueError:
            ...
            # ── Locate ───────────────────────────────────────────────────
                ...
                try:
                    x   = int(_eval(s.args["x"], self.variables, self.angle_mode))
                    y   = int(_eval(s.args["y"], self.variables, self.angle_mode))
                    txt = s.args["text"]
                    if txt.startswith('"') and txt.endswith('"'):
                        txt = txt[1:-1]
                    else:
                        txt = _fmt(_eval(txt, self.variables, self.angle_mode))
                    res.locate(x, y, txt)
                except ValueError as e:
                    res.error = str(e)
                    return
            ...
            # ── For loop ─────────────────────────────────────────────────
                ...
                try:
                    var   = s.args["var"]
                    start = _eval(s.args["start"], self.variables, self.angle_mode)
                    end   = _eval(s.args["end"],   self.variables, self.angle_mode)
                    step  = _eval(s.args["step"],  self.variables, self.angle_mode)
                    if step == 0:
            ...
            # ── While loop ───────────────────────────────────────────────
                ...
                try:
                    if _eval_cond(s.args["cond"], self.variables, self.angle_mode):
            ...
            # ── Do / LpWhile ─────────────────────────────────────────────
                ...
                try:
                    cont = _eval_cond(s.args["cond"], self.variables, self.angle_mode)
            ...
            # ── If block ─────────────────────────────────────────────────
                ...
                try:
                    cond_val = _eval_cond(s.args["cond"], self.variables, self.angle_mode)
                    if cond_val:
                        ip += 1
                    else:
            ...
            # ── ViewWindow ───────────────────────────────────────────────
                ...
                try:
                    for k, v in zip(keys, s.args["args"]):
                        self.view[k] = _eval(v, self.variables, self.angle_mode)
                except ValueError as e:
                    res.error = str(e)
                    return
                ip += 1
            ...
            # ── PlotOn / PlotOff / PlotChg ───────────────────────────────
                ...
                try:
                    px  = _eval(s.args["x"], self.variables, self.angle_mode)
                    py  = _eval(s.args["y"], self.variables, self.angle_mode)
            ...
            # ── F-Line ───────────────────────────────────────────────────
                ...
                try:
                    x1 = _eval(s.args["x1"], self.variables, self.angle_mode)
                    y1 = _eval(s.args["y1"], self.variables, self.angle_mode)
                    x2 = _eval(s.args["x2"], self.variables, self.angle_mode)
                    y2 = _eval(s.args["y2"], self.variables, self.angle_mode)
            ...
            # ── Getkey ───────────────────────────────────────────────────
                ...
                self.variables["ANS"] = 0.0   # 0 = no key in batch mode
                ip += 1

            # ── Dim List / List access ────────────────────────────────────
            elif s.cmd == "DIM_LIST":
                try:
                    n = int(_eval(s.args["n"], self.variables, self.angle_mode))
    ...
    def _exec_inline(
        ...
        # Other commands (nested If, loops) not supported inline — skip silently
        return None
